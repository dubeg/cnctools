...
namespace OpenRA
{
	...
	{
        public static string Join(this string s, string[] j)
        {
        ...
        public static string Join(this string ss, IEnumerable<string> ts)
        {
            foreach (var s in ts)
            {
                ss = s + ss;
        ...
        public static string JoinWith<T>(this string ss, IEnumerable<T> ts)
        {
            foreach (var s in ts)
            {
                ss = s.ToString() + ss;
		...
		public static Dictionary<TKey, TElement> ToDictionaryWithConflictLog<TSource, TKey, TElement>(this IEnumerable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TElement> elementSelector, string debugName, Func<TKey, string> logKey, Func<TElement, string> logValue)
		{
			...
			if (dupKeys.Count > 0)
			{
				var badKeysFormatted = string.Join(", ", dupKeys.Select(p => "{0}: [{1}]".F(logKey(p.Key), string.Join(",", p.Value.ToArray()).ToArray() )).ToArray() );
